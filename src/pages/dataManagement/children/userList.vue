    ...
    <index-header></index-header>
      <el-main>
        <el-table v-loading = 'loading' :data = "tableData" style="width: 100%" :default-sort = "{prop: 'date', order: 'descending'}" stripe border>
          ...
          <el-table-column prop = "admin" label = "角色权限" width = '180' :filters = "[{ text: '普通用户', value: '普通用户'}, { text: '管理员', value: '管理员'}]" :filter-method = 'filterTag' filter-placement = 'bottom-end'>
...
import {allUser, allUserCount} from '@/api/api'
export default {
  data () {
    return {
      tableData: [],
  ...
  methods: {
    ...
    async getData () {
      ...
      const res = await allUser(this.limit, this.offset)
      if (res) {
        this.loading = false
        res.forEach(item => {
          const userInfo = {}
          userInfo.user_name = item.username
          userInfo.create_time = item.registe_time
